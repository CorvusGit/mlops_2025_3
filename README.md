...
5 Скрипт очистки (clean_data2.py). 
    ...
    У части признаков некорректные значения заменяются на спец. заполнитель (-1), так как в них наблюдался фрод (чтоб не потерять)
    По части колонок, записи с некорректными значениями удаляются.
...
6 Запускал скрипт на кластере: s3-c4-m16 + 3 х s3-c8-m32. 
    ...
    то есть удалено 27252 + 2553 =  29805 строк , оставлены со спец значнеием (-1) 14623 некорректных customer_id и 13060 некорректных terminal_id (проверил на новом датасете - количество удаленных сходится)
...
PS. Думаю, так как количество отфильтрованных записей относительно небольшое, можно было бы сразу удалять некорректные данные, тогда работало бы сильно быстрее, не потребовалось persist.
Возможно два отдельных прохода оказались бы даже быстрее :)
