...
def get_coalesce_number(spark, logger, hdfs_path, target_size_mb=512,zip_coeff=3):
    ...
    logger.warn(f"Количество файлов в источнике: {file_count}")
    logger.warn(f"Общий размер: {total_mb:.2f} MB")
    logger.warn(f"Рекомендованный coalesce({n_coalesce}) для целевого размера {target_size_mb} MB")
